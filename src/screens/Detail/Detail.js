...
const Detail = ({route}) => {
  ...
  if(loading || typeof movie !== 'object'){
    return(
      <SafeAreaView
        style={theme === 'light' ? styles.lightLoadingContainer : styles.darkLoadingContainer}>
      ...
      {error ? 
        ...
        <>
          ...
          <View style={styles.detailWrapper}>
            ...
            <Text
              style={
                theme === 'light' ? styles.lightDescription : styles.darkDescription
              }>
              {movie.overview}
            </Text>
            ...
            <Text style={theme === 'light' ? styles.lightHeader : styles.darkHeader}>Genres:</Text>
            <View style={styles.genresWrapper}>
              {movie.genres.map((item,index)=>
                <Text key={item.id} style={theme === 'light' ? styles.lightGenre : styles.darkGenre}>{item.name+ (movie.genres.length-1!==index ? ' | ':'') }</Text>
              )}
            </View>
            <Text style={theme === 'light' ? styles.lightHeader : styles.darkHeader}>Companies:</Text>
